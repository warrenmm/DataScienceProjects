...
###         Project 3 code is at the bottom of the page. 
...
y1 = lm (abs_logerror ~ id_parcel+build_year+area_total_calc+area_garage+area_lot+num_bathroom+num_bedroom+num_bathroom_calc+num_bath+num_garage+num_room+flag_tub+flag_fireplace+region_city+region_county+region_neighbor+region_zip+fips+heating+latitude+longitude+rawcensustractandblock+tax_building+tax_total+tax_year+tax_land+tax_property+censustractandblock, data=cor_tmp)
summary(y1)
y2 = lm (logerror ~ id_parcel+build_year+area_total_calc+area_garage+area_lot+num_bathroom+num_bedroom+num_bathroom_calc+num_bath+num_garage+num_room+flag_tub+flag_fireplace+region_city+region_county+region_neighbor+region_zip+fips+heating+latitude+longitude+rawcensustractandblock+tax_building+tax_total+tax_year+tax_land+tax_property+censustractandblock, data=cor_tmp)
summary(y2)

#After running a regression of all available data with respect to log error and absolute log error,
#I found that while no variable registered as statistically significant predictors of log error, 
#regression of absolute log error shows that there are several good predictors. The discrepancy 
#in results between logerror and abs_logerror is likely because positive and negative values of 
#logerror counterbalance each other, meaning that a confounding variable might contribute little
#net log error. Statistically significant predictors of abs_logerror are "id_parcel," "num_bedroom,"
#"region_city," "region_county," "heating," and "rawcemsistractandblock".   Zillow could improve 
#their price estimation program by refining their model where these factors are concerned. 